...
Class Action
{
    ...
    function save_project()
    {
        ...
        if (empty($id)) {
            ...
            $_SESSION['project_id'] = $this->db->insert_id;
        } else {
    ...
    function delete_project()
    {
        ...
        $notification = $this->db->query("DELETE FROM users_notifications where project_id = $id");
        if ($delete && $notification) {
            return 1;
        }
    }
    ...
    function new_notification()
    {
        ...
        $data = "";
        foreach ($_POST as $k => $v) {
            if (!in_array($k, array('id', 'manager_id', 'user_id')) && !is_numeric($k)) {
                if ($k == 'status') {
                    $v = 0;
                }
                if ($k == 'project_id') {
                    $v = $_SESSION['project_id'];
                }
                if (empty($data)) {
                    $data .= " $k='$v' ";
                } else {
        ...
        if (isset($user_id)) {
            foreach ($user_id as $value) {
                $data .= ", user_id = " . $value;
                $save = $this->db->query("INSERT INTO users_notifications set $data");
                $data = str_replace(", user_id = " . $value, "", $data);
            }
            if (!empty($manager_id)) {
