...
<div class="col-lg-12">
					...
					<div class="row">
						...
						<div class="col-md-6">
							<dl>
								...
								<dd><?php getTime(strtotime($start_date)) ?></dd>
							</dl>
							<dl>
								<dt><b class="border-bottom border-primary">Ngày kết thúc</b></dt>
								<dd><?php getTime(strtotime($end_date)) ?></dd>
							</dl>
							<dl>
								...
								<dd>
									<?php
									  if($stat[$status] =='Pending'){
									  	echo "<span class='badge badge-secondary'>Đang chờ</span>";
									  }elseif($stat[$status] =='Started'){
									  	echo "<span class='badge badge-primary'>Bắt đầu</span>";
									  }elseif($stat[$status] =='On-Progress'){
									  	echo "<span class='badge badge-info'>Đang tiến hành</span>";
									  }elseif($stat[$status] =='On-Hold'){
									  	echo "<span class='badge badge-warning'>Tạm dừng</span>";
							...
							<dl>
								...
								<dd>
									...
									<?php else: ?>
										<small><i>Đã xóa khỏi cơ sở dữ liệu</i></small>
									<?php endif; ?>
								</dd>
							</dl>
						</div>
		...
		<div class="col-md-4">
			<div class="card card-outline card-primary">
				<div class="card-header">
					<span><b>Thành viên trong nhóm</b></span>
				...
				<div class="card-body">
					<ul class="users-list clearfix">
						<?php 
						if(!empty($user_ids)):
							$members = $conn->query("SELECT *,concat(lastname,' ',firstname) as name FROM users where id in ($user_ids) order by concat(lastname,' ',firstname) asc");
	...
	<div class="row">
		<div class="col-md-12">
			<div class="card">
				<div class="card-header">
					<b>Tiến độ thành viên</b>
					<div class="card-tools">
						<button class="btn btn-primary bg-gradient-primary btn-sm" type="button" id="new_productivity"><i class="fa fa-plus"></i> Tiến độ mới</button>
					</div>
				</div>
				<div class="card-body">
					<?php 
					$progress = $conn->query("SELECT p.*,concat(u.lastname,' ',u.firstname) as uname,u.avatar,t.task FROM user_productivity p inner join users u on u.id = p.user_id inner join task_list t on t.id = p.task_id where p.project_id = $id order by unix_timestamp(p.date_created) desc ");
						...
						<div class="post">
		                      ...
		                      <div class="user-block">
		                        ...
		                        <span class="description">
		                        	...
		                        	<span><b><?php getTime(strtotime($row['date']))  ?> </b></span>
...
<script>
	$('#new_task').click(function(){
		uni_modal("Nhiệm vụ mới cho <?php echo ucwords($name) ?>","manage_task.php?pid=<?php echo $id ?>","mid-large")
	})
	$('.edit_task').click(function(){
		uni_modal("Sửa nhiệm vụ: "+$(this).attr('data-task'),"manage_task.php?pid=<?php echo $id ?>&id="+$(this).attr('data-id'),"mid-large")
	})
	$('.view_task').click(function(){
		uni_modal("Chi tiết nhiệm vụ","view_task.php?id="+$(this).attr('data-id'),"mid-large")
	})
	...
	_conf("Bạn có chắc muốn xóa tiến độ này?","delete_progress",[$(this).attr('data-id')])
	})
